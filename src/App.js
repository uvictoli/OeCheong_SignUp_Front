    ...
    if (key === 'phoneNum') {
      ...
      if (phoneNum.length > 0 && false === phoneReg.test(phoneNum)) {
        ...
        axios({
          ...
          data: { student_num: e.target.value }
        })
        .then((res) => {
          if (res.data !== null) {
        ...
        <ul>
          ...
          <li>
            <label htmlFor="student number">
              <div>학번</div>
              <input type="number" name="studentNum" placeholder=' ex) 202112345' onChange={this.handleSignUpValue('studentNum')}></input>
          ...
          <li>
            <label htmlFor="password">
              <div>비밀번호</div>
              <input type="passwiod" name="pw" placeholder=' 영어/숫자/특수문자 모두 포함 (8글자~15글자)' onChange={this.handleSignUpValue('pw')}></input>
